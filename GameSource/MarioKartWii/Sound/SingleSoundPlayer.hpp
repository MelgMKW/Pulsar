...
public:
    ...
    AudioHandle* PlaySound(u32 soundId, s16 delay); //806f8b7c
    ...
    AudioHandle* PrepareSound(u32 soundId, bool cannotBeOverwritten); //806f8dcc r5 == 0x24 bool

    //Plays the prepared sound irrespective of its soundId (no id arg) by moving it to activeHandle
    AudioHandle* PlayPreparedSound(s16 delay); //806f9034

    void BlockActiveSoundChange(); //806f90b8 simply sets canNotCancel
    void StopSound(); //806f90c4 Stops the activeHandle
    bool StartSound(AudioHandle* handle, u32 soundId); //806f913c
    void SetupActiveHandle(AudioHandle* handle); //806f933c pauses the BasicSound of the handle and sets up MainVolumeMgr/StreamsMgr

    //Starts the activeHandle irrespective of any delay
    AudioHandle* StartActiveSound(); //806f95d8

    //Finds the 1st free handle, and if there isn't one, stops prepared sound and uses that one
    AudioHandle* GetFreeHandle(); //806f9698 inlined
    ...
    EGG::TDisposer<SingleSoundPlayer> disposer; //806f84a8 vtable 808c78ec
