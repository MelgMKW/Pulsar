...
public:
    ...
    static File *CreateStaticInstance(EGG::TaskThread *taskThread);
    File(EGG::TaskThread *taskThread): fd(-1), taskThread(taskThread) { memset(&this->path, 0, IPCMAXPATH); }
...
public:
    RiivoFile(EGG::TaskThread *taskThread): File(taskThread) {};
