...
void CreateUMT(KartMovement *movement) {
    ...
    if (smtCharge >= 550 && Pulsar::sInstance->HasUMTs()) movement->driftState = 4;
...
void CreatePlayerEffects(EffectsMgr *mgr) { //adding the resource here as all other breff have been loaded at this point
    if (Pulsar::sInstance->HasUMTs()) {
        ArchiveRoot *root = ArchiveRoot::sInstance;
        void *breff = root->GetFile(ARCHIVE_HOLDER_COMMON, Pulsar::breff, 0);
        void *breft = root->GetFile(ARCHIVE_HOLDER_COMMON, Pulsar::breft, 0);
    ...
    for (int i = 0; i < RaceData::sInstance->racesScenario.playerCount; i++) {
        mgr->playersEffects[i] = new(ExpPlayerEffects)(KartHolder::sInstance->GetKart(i));
    }
}
...
void LoadCustomEffects(ExpPlayerEffects *effects) {
    effects->LoadEffects();
    if (effects->isBike == false && Pulsar::sInstance->HasUMTs()) {
...
void LoadLeftPurpleSparkEffects(ExpPlayerEffects *effects, EGG::Effect **effectArray, u32 firstEffectIndex, u32 lastEffectIndex, Mtx34 *playerMat2, Vec3 *wheelPos, bool r9) {
    u32 smtCharge = effects->kart->base.pointers->kartMovement->smtCharge;
    if (smtCharge >= 550 && Pulsar::sInstance->HasUMTs()) effects->DisplayPrimaryEffects(effects->rk_purpleMT, 0, 2, playerMat2, wheelPos, r9);
...
void LoadRightPurpleSparkEffects(ExpPlayerEffects *effects, EGG::Effect **effectArray, u32 firstEffectIndex, u32 lastEffectIndex, Mtx34 *playerMat2, Vec3 *wheelPos, bool r9) {
    u32 smtCharge = effects->kart->base.pointers->kartMovement->smtCharge;
    if (smtCharge >= 550 && Pulsar::sInstance->HasUMTs()) effects->DisplayPrimaryEffects(effects->rk_purpleMT, 2, 4, playerMat2, wheelPos, r9);
    else effects->DisplayPrimaryEffects(effectArray, firstEffectIndex, lastEffectIndex, playerMat2, wheelPos, r9);
};
kmCall(0x80698af0, LoadRightPurpleSparkEffects);

//Fade the sparks
void FadeLeftPurpleSparkEffects(ExpPlayerEffects *effects, EGG::Effect **effectArray, u32 firstEffectIndex, u32 lastEffectIndex, Mtx34 *playerMat2, Vec3 *wheelPos, bool r9) {
    if (Pulsar::sInstance->HasUMTs()) effects->FadeEffects(effects->rk_purpleMT, 0, 2, playerMat2, wheelPos, r9);
...
void FadeRightPurpleSparkEffects(ExpPlayerEffects *effects, EGG::Effect **effectArray, u32 firstEffectIndex, u32 lastEffectIndex, Mtx34 *playerMat2, Vec3 *wheelPos, bool r9) {
    if (Pulsar::sInstance->HasUMTs()) effects->FadeEffects(effects->rk_purpleMT, 2, 4, playerMat2, wheelPos, r9);
    effects->FadeEffects(effectArray, firstEffectIndex, lastEffectIndex, playerMat2, wheelPos, r9);
};
...
void PatchBoostOnUMTSpeedBoost(EGG::Effect *boostEffect, u8 loopIndex, ExpPlayerEffects *effects) { //have to mod loop index by 4 to get the actual index
...
void PatchBoostMatrix(EGG::Effect *boostEffect, Mtx34 *boostMat, u8 loopIndex, ExpPlayerEffects *effects) {
    //if (umtState[effects->playerId]) boostEffect = effects->rk_purpleMT[rk_purpleBoost + loopIndex%4];
    if (boostEffect->handleBase.GetPtr()) {
        boostEffect->SetMtx(*boostMat);
        boostEffect->Update();
    }
    if (!effects->isBike && Pulsar::sInstance->HasUMTs()) {
...
void PatchFadeBoost(EGG::Effect *boostEffect, u8 loopIndex, ExpPlayerEffects *effects) {
    boostEffect->FollowFade();
    if (!effects->isBike && Pulsar::sInstance->HasUMTs()) effects->rk_purpleMT[rk_purpleBoost + loopIndex % 4]->FollowFade();
}
