...
class ObjectCollidable: public Object { //collision is added as there is no KCL, different shapes available cube/cylinder/sphere, depends on the obj
    explicit ObjectCollidable(const KMP::GOBJHolder& gobjHolder); //8081efec
    ...
    virtual void BeforeKartCollision(const Kart& kart, ObjToKartHit* objToKart, KartToObjHit& kartToObj); //0xb8 8081f66c
    ...
    virtual ObjToKartHit OnCollision(const Kart& kart, ObjToKartHit default, KartToObjHit kartToObj) const; //0xc0 8068179c
    //depends on factors like speed and obviously the kartToObj as a goomba does not do anything to a player in a mega
    virtual ObjToItemInteraction GetItemCollisionType(const Kart& kart,
        ObjToItemInteraction default, ItemToObjInteraction itemToObj) const; //0xc4 806817a4
    //same as above but with items
    virtual void OnWallCollision(const Kart& kart, Vec3ZeroInit position); //0xc8 8081f548 sound and playereffectsx
    //for objects that behave like walls such as goombas at low speeds
    virtual void OnNormalCollision(const Kart& kart, Vec3ZeroInit position); //0xcc 8081f5d8 sound and playereffects
