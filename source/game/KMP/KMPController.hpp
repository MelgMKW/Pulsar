...
public:
    ...
    bool IsPointInAREA(const Vec3& position); //805160b0
    AREA* rawData;
    Vec3 xVector;
    Vec3 yVector;
    Vec3 zVector;
...
struct JGPTHolder: EntryHolder<JGPT> {
    Vec3 rotation;
    Vec3 zScale;
    Vec3 xScale;
    u8 enemyLink;
...
struct GOBJHolder: EntryHolder<GOBJ> {
    const POTIHolder* GetRoute() const; //80514194
};
...
public:
    ...
    void Init(); //805127ec
    const POTIHolder* GetPOTIHolder(u8 idx) const; //80515e04
    const GOBJHolder* GetGOBJHolder(u8 idx) const; //80514148
    const AREAHolder* GetAREAHolder(u8 idx) const; //80516768
    const KTPTHolder* GetKTPTHolder(u8 idx) const; //80514b30
    const JGPTHolder* GETJPGTHolder(u8 idx) const; //80518920
    ...
    SectionHolder<JGPTHolder, JGPT>* respawn;     //34
